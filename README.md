...
2. **突然弹出将近 50 个小窗口**密密麻麻覆盖整个屏幕——终端乱码、系统错误、崩溃报告、矩阵字符、蓝屏、十六进制dump，模拟一场系统全面崩溃的混乱
3. **弹窗逐个关闭**，像故障被逐个修复
4. **主窗口在弹窗全部退去后从底层淡入显现**，仪式感拉满
...
### 修复 & 优化
- [x] 弹窗数量增加到 **45-55**，网格分布算法确保几乎看不到桌面
- [x] 弹窗全部改为无边框样式，内容区域最大化
- [x] 所有弹窗内容纯静态渲染，零卡顿
- [x] 新增蓝屏视图和十六进制 dump 视图
- [x] 主窗口使用 `orderBack` 确保在弹窗最底层，零闪烁
- [x] 修复全局呼出快捷键注册/录制问题
