...
2. **突然弹出 50 个小窗口**密密麻麻覆盖整个屏幕——终端乱码、系统错误、崩溃报告、矩阵字符、蓝屏、十六进制dump，模拟一场系统全面崩溃的混乱
3. **系统错误音效连环轰炸**——Basso、Funk、Glass、Tock 等错误音效叠加播放
4. **主窗口混在第 3-4 个弹窗中一起诞生**，从底层慢慢显现，和混乱融为一体
5. **弹窗逐个关闭**，主窗口在弹窗退去的过程中逐渐清晰，最终完全显现
...
### 修复 & 优化
- [x] 弹窗数量固定为 **50**，网格分布确保几乎看不到桌面
- [x] 弹窗全部改为无边框样式，内容区域最大化
- [x] 所有弹窗内容纯静态渲染，零卡顿
- [x] 新增蓝屏视图和十六进制 dump 视图
- [x] **主窗口混入弹窗一起显现**，不是最后才出现
