...
### 修复
- 修复 `AppPreferences.default` 参数顺序导致的编译错误，并为旧版设置增加缺省值迁移，避免升级后配置被重置
- 修复全局呼出快捷键的 Cocoa/Carbon 修饰符转换，避免 ⌘⇧C 等快捷键注册后无法触发
- 修复 Carbon 热键事件处理器误吞其他热键事件的问题，并补齐退出时的 handler 清理
- 修复设置页录制全局快捷键时前台窗口无法捕获按键的问题
- 修复 F1-F12 作为标签快捷键时无法无修饰键保存、无法正确冲突检测的问题
- 修复浏览器已启动但没有窗口时无法新建标签打开目标 URL 的问题
- 修复清空全部标签后主面板列表和已注册快捷键不同步的问题
- 修复打开主面板时音效重复播放的问题
- 修复空状态页图标使用系统强调色（蓝色）的问题，统一为黑白主题白色
- 修复自动探测当前标签时误弹 Toast 的问题（静默探测不应打扰用户）
- 修复 Toast 在主面板关闭后残留、下次打开仍显示的问题
- 修复动画速度实际值与设定不符的问题（Fast 0.08s → 0.03s，Normal 0.2s → 0.1s）

### 优化
- 补齐面板圆角设置入口，让 v0.3 的圆角参数真正可调
- 补齐手动添加标签入口，保留“一键保存当前标签”的快速流程
- 显示面板自动探测当前标签时增加去重，避免重复保存同一页面
- 设置主题、标签数量徽章、切换延迟、默认浏览器、清空确认等 v0.3 参数现在都已接入实际行为
- 菜单栏图标标签数量徽章（`showTabCountBadge`）真正生效：启用后在菜单栏图标右上角显示红色数字徽章
- TabRow 键盘导航焦点高亮：启用 `enableKeyboardNavigation` 后，Tab 键聚焦的标签会有明显的白色高亮背景
- 导出设置时默认文件名改为带时间戳的完整 JSON 文件名
- 项目部署目标调整为 macOS 14.0，和 README / 开发指南保持一致
- 版本号更新为 v0.3.1 (Build 4)
