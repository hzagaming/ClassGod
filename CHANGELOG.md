...
### 修复
- **Helper 启动后自动重新扫描**：点击 "Start Helper" 后等待 3 秒，自动断开旧连接、`rescan()`、刷新 UI，不需要手动点 rescan
- **增强 Helper 枚举日志**：启动时打印 `#KEY` 读取结果、硬编码键成功率、动态 T 键数量，方便排查 M5 Pro 键兼容性问题
- **放宽动态枚举类型过滤**：除 `sp78` 等标准温度类型外，也接受任意 `T`/`F` 前缀键，避免 M5 Pro 使用不同 SMC 类型码时漏掉传感器
- 版本号更新为 v1.5.0 (Build 25)
