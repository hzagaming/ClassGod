...
- `SMCService`：通过 IOKit 读取 SMC 温度传感器和风扇转速，支持 Intel / Apple Silicon；支持风扇模式切换（System / Max / Auto Max）
...
- `FanControlView` / `FanControlViewModel`：风扇控制面板，含温度列表、风扇转速条、诊断信息
- `FanControlSettingsView`：风扇控制设置页（更新间隔、温度单位、Auto Max 阈值）
...
- Apple Silicon 机型上 SMC 风扇控制可能需要 root 权限，部分功能可能受限
